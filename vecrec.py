...
def vecrec(prev, cap):
    vehile = 0
    w=  prev.shape[1]
    nxt=imutils.resize(prev.copy(), height=600,width=w)

    framer = imutils.resize(cap.copy(), height=600,width=w)
    frame = framer
    ...
    for c in contours:
        ...
        # remove the distraction on the side of frames consider only the objects on entry/exit
        if y > 200:
